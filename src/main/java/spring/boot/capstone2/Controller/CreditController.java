...
public class CreditController {
    ...
    @PostMapping("/add")
    public ResponseEntity addCredit(@Valid @RequestBody Credit credit) {
        creditService.addCredit(credit);
        return ResponseEntity.status(201).body("Credit added");
    }

    @PutMapping("/update/{id}")
    public ResponseEntity updateCredit(@Valid @RequestBody Credit credit, @PathVariable Integer id ) {
